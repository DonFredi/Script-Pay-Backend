...
export class WebhookPollerService {
  ...
  private async processEvent(event: { id: string; source: string; payload: unknown; attempts: number }) {
    try {
      ...
      } else if (event.source === "daraja_transaction_status_result") {
        await this.processTransactionStatusResult(event.payload as any);
      } else if (event.source === "daraja_transaction_status_timeout") {
        await this.processTransactionStatusTimeout(event.payload as any);
      }
  ...
  /**
   * Auto-recovery counterpart to processB2cResult, for DriftDetectorService's status
   * query on a payout stuck without a real B2C callback. CRITICAL difference: this
   * callback's OriginatorConversationID belongs to the STATUS QUERY itself (fresh,
   * generated by Safaricom when the query was accepted), never the original stuck
   * payout — confirmed empirically (docs/decisions.md entry 41). Correlates via
   * PayoutStatusQuery, written by DriftDetectorService before the query went out.
   *
   * STAGE 1 (deliberately not applying TransactionStateMachine yet): the exact
   * `TransactionStatus` string Safaricom sends for a completed vs failed payout was
   * not confirmed against a real captured payload before this shipped — every source
   * consulted described the field's existence but not its enumerated values. Rather
   * than guess a mapping on a money-recovery path (exactly what docs/decisions.md
   * entry 18 warned against), this records the raw value for inspection and leaves
   * the payout untouched. A follow-up wires the confirmed mapping into
   * transitionPayoutToSettled/transitionPayoutToFailed once real data exists.
   */
  private async processTransactionStatusResult(payload: any) {
    const result = payload?.Result;
    const queryOriginatorConversationId = result?.OriginatorConversationID;

    if (!queryOriginatorConversationId) {
      this.logger.warn("Transaction status result carried no OriginatorConversationID — ignoring");
      await this.auditLog.record({
        actorType: "system",
        action: "daraja.transaction_status_unmatched",
        metadata: { reason: "missing_originator_conversation_id" },
      });
      return;
    }

    const query = await this.prisma.payoutStatusQuery.findUnique({
      where: { queryOriginatorConversationId },
    });

    if (!query) {
      this.logger.warn(`Transaction status result for unknown query ${queryOriginatorConversationId} — ignoring`);
      await this.auditLog.record({
        actorType: "system",
        action: "daraja.transaction_status_unmatched",
        metadata: { queryOriginatorConversationId },
      });
      return;
    }

    const params = result?.ResultParameters?.ResultParameter ?? [];
    const statusParam = params.find((p: any) => p.Key === "TransactionStatus");
    const transactionStatus = typeof statusParam?.Value === "string" ? statusParam.Value : null;

    await this.prisma.payoutStatusQuery.update({
      where: { id: query.id },
    ...
    await this.auditLog.record({
      ...
      targetType: "Transaction",
      targetId: query.transactionId,
      metadata: { queryOriginatorConversationId, resultCode: result?.ResultCode, transactionStatus, rawResultParameters: params },
    });

    this.logger.warn(
      `Transaction status result received for payout ${query.transactionId} — TransactionStatus="${transactionStatus ?? "absent"}". ` +
        "Not yet applied to the transaction (auto-recovery mapping unconfirmed against real data) — " +
        "see the daraja.transaction_status_result_received audit log entry for the raw ResultParameters.",
    );
  }

  /**
   * The STATUS QUERY itself timed out — not the underlying payout, which is
   * unaffected either way. DriftDetectorService's own critical alert (fired when the
   * query was sent) is still what a human acts on; this just closes out the query
   * row so a future pass doesn't consider it perpetually in-flight.
   */
  private async processTransactionStatusTimeout(payload: any) {
    const result = payload?.Result;
    const queryOriginatorConversationId = result?.OriginatorConversationID;

    const query = queryOriginatorConversationId
      ? await this.prisma.payoutStatusQuery.findUnique({ where: { queryOriginatorConversationId } })
      : null;

    this.logger.warn(
      `Transaction status query timed out for ${queryOriginatorConversationId ?? "(missing)"} — ` +
        "the underlying payout is unaffected",
    );
    ...
    await this.auditLog.record({
      tenantId: query?.tenantId,
      actorType: "system",
      action: "daraja.transaction_status_query_timeout",
      targetType: query ? "Transaction" : undefined,
      targetId: query?.transactionId,
      metadata: { queryOriginatorConversationId },
    });
  }
